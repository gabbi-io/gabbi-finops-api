...
def json_safe(obj):
    """Converte tipos vindos do PostgreSQL para estruturas serializáveis em JSON."""
...
def api_json(obj):
    """Wrapper único para evitar erro Decimal/date/datetime not JSON serializable."""
    return jsonify(json_safe(obj))
...
def api_state():
    state = _state()
    return api_json(_get_dataset(state))
...
def api_reset():
    ...
    _save(state)
    return api_json({"ok": True, "message": "Estado resetado (live).", "state": _get_dataset(state)})
...
def api_step():
    ...
    if state.get("data_source") == "real":
        return api_json({"ok": False, "message": "Modo REAL: simulação desabilitada.", "state": _get_dataset(state)}), 400

    out = sim.step_live(state, steps=steps, spike=spike)
    _save(state)
    return api_json({"ok": True, "result": out, "state": _get_dataset(state)})
...
def api_spike():
    ...
    if state.get("data_source") == "real":
        return api_json({"ok": False, "message": "Modo REAL: spike desabilitado.", "state": _get_dataset(state)}), 400
...
def api_finops_usage():
    ...
    try:
        ...
        return api_json(out), (200 if out.get("ok") else 400)
    except Exception as e:
        return api_json({"ok": False, "error": "exception", "message": str(e)}), 500
...
def api_finops_pricing():
    ...
    if not model or cost is None:
        return api_json({"ok": False, "error": "missing_field:model|cost_per_1k_tokens_brl"}), 400
